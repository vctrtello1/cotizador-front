<template>
    <div class="editar-conector-container">
        <div class="page-header">
            <button class="btn-back" @click="$router.push('/conectores')" title="Regresar">
                ← Regresar
            </button>
            <h1 class="page-title">✏️ Editar Conector</h1>
        </div>

        <div v-if="loading && !formData.nombre" class="loading-message">Cargando conector...</div>
        <form v-else @submit.prevent="guardarConector" class="conector-form">
            ...
            <div class="form-actions">
                <button type="button" @click="$router.push('/conectores')" class="btn-secondary">
                    Cancelar
                </button>
...
// Cargar conector
const cargarConector = async () => {
    loading.value = true;
    try {
        const response = await conectoresStore.fetchConectorPorId(route.params.id);
        ...
        console.error('Error al cargar conector:', error);
        alert('Error al cargar el conector');
...
const guardarConector = async () => {
    ...
    try {
        ...
        await conectoresStore.actualizarConectorExistente(route.params.id, datos);
        router.push('/conectores');
    } catch (error) {
        console.error('Error al actualizar conector:', error);
