<template>
    <div class="nuevo-conector-container">
        <div class="page-header">
            <button class="btn-back" @click="$router.push('/conectores')" title="Regresar">
                ← Regresar
            </button>
            <h1 class="page-title">🔗 Nuevo Conector</h1>
        </div>

        <form @submit.prevent="guardarConector" class="conector-form">
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn-primary" :disabled="loading">
                    {{ loading ? 'Guardando...' : 'Guardar Conector' }}
                </button>
            </div>
        </form>
    </div>
...
const guardarConector = async () => {
    ...
    try {
        ...
        await conectoresStore.crearConectorNuevo(datos);
        router.push('/conectores');
    } catch (error) {
        console.error('Error al guardar conector:', error);
        alert('Error al guardar el conector');
    } finally {
        loading.value = false;
    }
};
...
onMounted(async () => {
    ...
    const count = conectoresStore.conectores.length;
    formData.value.nombre = `CONECTOR ${count + 1}`;
});
</script>

<style scoped>
.nuevo-conector-container {
    max-width: 800px;
    margin: 0 auto;
    padding: 2rem;
    min-height: 100vh;
