<template>
    <div class="form-container">
        ...
        <div class="form-header">
            <h1 class="form-title">💡 Nueva Tira LED</h1>
        </div>
        ...
        <form @submit.prevent="guardarTiraLed" class="form-content">
...
const guardarTiraLed = async () => {
    ...
    try {
        ...
        await tirasLedStore.crearTiraLed(datos);
        exito.value = 'Tira LED creada correctamente';

        setTimeout(() => {
            router.push('/tiras-led');
        }, 1500);
    } catch (err) {
        error.value = 'Error al guardar la tira LED';
        console.error(err);
    } finally {
...
onMounted(async () => {
    ...
    try {
        ...
        const tirasLedExistentes = tirasLedStore.tirasLed;
        numeroTiraLed = tirasLedExistentes.length + 1;
    } catch (err) {
        console.error('Error contando tiras LED existentes:', err);
    }

    formData.value.nombre = `TIRA LED ${numeroTiraLed}`;
});
</script>
