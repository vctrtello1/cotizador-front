...
const guardarAcabado = async () => {
    ...
    try {
        ...
        await acabadosStore.crearAcabado(datos);
        exito.value = 'Acabado creado correctamente';
...
onMounted(async () => {
    ...
    try {
        ...
        const acabadosExistentes = acabadosStore.acabados;
        numeroAcabado = acabadosExistentes.length + 1;
    } catch (err) {
        console.error('Error contando acabados existentes:', err);
    }

    formData.value.nombre = `ACABADO ${numeroAcabado}`;
});
</script>
