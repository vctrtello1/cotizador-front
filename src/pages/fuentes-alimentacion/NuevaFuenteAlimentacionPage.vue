<template>
    <div class="nueva-fuente-container">
        <div class="page-header">
            <button class="btn-back" @click="$router.push('/fuentes-alimentacion')" title="Regresar">
                ← Regresar
            </button>
            <h1 class="page-title">🔌 Nueva Fuente de Alimentación</h1>
        </div>

        <form @submit.prevent="guardarFuente" class="fuente-form">
            ...
            <div class="form-actions">
                ...
                <button type="submit" class="btn-primary" :disabled="loading">
                    {{ loading ? 'Guardando...' : 'Guardar Fuente de Alimentación' }}
                </button>
            </div>
        </form>
    </div>
...
const guardarFuente = async () => {
    ...
    try {
        ...
        await fuentesStore.crearFuente(datos);
        router.push('/fuentes-alimentacion');
    } catch (error) {
        console.error('Error al guardar fuente de alimentación:', error);
        alert('Error al guardar la fuente de alimentación');
    } finally {
        loading.value = false;
    }
};
...
onMounted(async () => {
    ...
    const count = fuentesStore.fuentesAlimentacion.length;
    formData.value.nombre = `FUENTE ${count + 1}`;
});
</script>

<style scoped>
.nueva-fuente-container {
    max-width: 800px;
    margin: 0 auto;
    padding: 2rem;
    min-height: 100vh;
