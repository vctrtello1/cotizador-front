<template>
    <div class="form-container">
        ...
        <div class="form-header">
            <h1 class="form-title">🔘 Nuevo Apagador</h1>
        </div>
        ...
        <form @submit.prevent="guardarApagador" class="form-content">
...
const guardarApagador = async () => {
    ...
    try {
        ...
        console.log('Guardando apagador:', datos);

        await apagadoresStore.crearApagador(datos);
        exito.value = 'Apagador creado correctamente';

        setTimeout(() => {
            router.push('/apagadores');
        }, 1500);
    } catch (err) {
        error.value = 'Error al guardar el apagador';
        console.error(err);
    } finally {
...
onMounted(async () => {
    let numeroApagador = 1;
    try {
        await apagadoresStore.fetchApagadores();
        const apagadoresExistentes = apagadoresStore.apagadores;
        numeroApagador = apagadoresExistentes.length + 1;
    } catch (err) {
        console.error('Error contando apagadores existentes:', err);
    }

    formData.value.nombre = `APAGADOR ${numeroApagador}`;
});
</script>
