<template>
    <div class="form-container">
        ...
        <div class="form-header">
            <h1 class="form-title">🔩 Nuevo Accesorio</h1>
        </div>
        ...
        <form @submit.prevent="guardarAccesorio" class="form-content">
            <div class="form-group">
                ...
                <input
                    ...
                    type="text"
                    id="nombre"
                    placeholder="Ej: Jaladera de acero"
                    required
                />
...
const guardarAccesorio = async () => {
    ...
    try {
        ...
        console.log('Guardando accesorio:', datos);

        await accesoriosStore.crearAccesorio(datos);
        exito.value = 'Accesorio creado correctamente';

        setTimeout(() => {
            router.push('/accesorios');
        }, 1500);
    } catch (err) {
        error.value = 'Error al guardar el accesorio';
        console.error(err);
    } finally {
...
onMounted(async () => {
    let numeroAccesorio = 1;
    try {
        await accesoriosStore.fetchAccesorios();
        const accesoriosExistentes = accesoriosStore.accesorios;
        numeroAccesorio = accesoriosExistentes.length + 1;
    } catch (err) {
        console.error('Error contando accesorios existentes:', err);
    }

    formData.value.nombre = `ACCESORIO ${numeroAccesorio}`;
});
</script>
